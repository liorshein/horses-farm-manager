...
const updateArrived = (lesson_id: string, boolean: string) => {
    ...
    axios.put(API + `/api/update-arrived?${params}`)
}
...
const addStudent = (inputs: Student) => {
    axios.post(API + "/api/add-student", {
...
const getPersonalInfo = async () => {
    return (await axios.get(API + "/api/user")).data.result
}

const getFavoriteHouse = async () => {
    let result = await axios.get(API + `/api/favorite-horse`);
    return result
};

const getSalaryPerMonth = async (date: string) => {
    let params = new URLSearchParams({ year_month_str: date })    
    let results = await (await axios.get(API + `/api/salary?${params}`)).data.result.rows[0].count
    return results
}

const getLessonsPerMonth = async () => {
    let results = await (await axios.get(API + `/api/lessons-per-month`)).data.result.rows
    return results
}

const getMonthOfLessons = async () => {
    let results = await (await axios.get(API + `/api/lessons-monthly`)).data.result.rows  
    return results
}

const getAvailableHours = async (horse_id: string, date: string) => {
    let params = new URLSearchParams({ horse_id: horse_id, date: date })
    let availableHours = await axios.get(API + `/api/lessons-available?${params}`);
    return availableHours
};

const getUserStudentsInfo = async () => {
    return (await axios.get(API + "/api/user-students")).data.result
}

const getHorsesInfo = async () => {
    return (await axios.get(API + "/api/horses")).data.result
}

const getAvailableHorses = async () => {
    return (await axios.get(API + "/api/horses-available")).data.result
}

const getUserLessons = async (date: string) => {
    let params = new URLSearchParams({ date: date })
    return (await axios.get(API + `/api/lessons?${params}`)).data.result
}
