...
export const getStudentsData: RequestHandler = async (req: any, res) => {
  const InstructorId = req.user
  const result = (
    await pool.query('SELECT * FROM students WHERE instructor_id = $1', [
      InstructorId,
    ])
  ).rows
  res.send({ result })
...
export const updateArrived: RequestHandler = async (req, _res) => {
  ...
  await pool.query(`UPDATE lessons SET arrived=$1 WHERE lesson_id=$2`, [
    booleanStr,
    lessonId,
  ])
}
...
export const getHorsesData: RequestHandler = async (_req, res) => {
  const result = (await pool.query('SELECT * FROM horses')).rows
  res.send({ result })
}
