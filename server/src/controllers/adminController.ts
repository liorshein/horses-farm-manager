...
export const deleteHorse: RequestHandler = async (req: any, res) => {
  ...
  try {
    await db.query(`DELETE FROM horses WHERE horse_id=$1`, [horseId])
...
export const getStudentsData: RequestHandler = async (req: any, res) => {
  ...
  const result = (
    await db.query('SELECT * FROM students WHERE instructor_id = $1', [Number(InstructorId)])
  ).rows
  res.send({ result })
}
...
export const getAllInstructorsData: RequestHandler = async (_req, res) => {
  const result = (
    await db.query(`SELECT * FROM instructors WHERE instructor_id > 1`)
  ).rows
  res.send({ result })
}
...
export const getAvailableHorses: RequestHandler = async (_req, res) => {
  const result = (
    await db.query("SELECT * FROM horses WHERE assignable='True'")
  ).rows
  res.send({ result })
}
